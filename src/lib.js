...
export function loadState() {
  ...
  try {
    ...
    if (rawV1) {
      ...
      if (Array.isArray(parsed)) {
        return { items: parsed.map(normalizeItem), name: '', greeting: DEFAULT_GREETING }
      }
    }
  } catch {
    /* fall through */
  ...
  // First run.
  return { items: makeSampleItems(), name: '', greeting: DEFAULT_GREETING }
}
...
// Outlook report
// ---------------------------------------------------------------------------
export function buildOutlookHtml(items, summary, name, greeting) {
  const signature = name?.trim() || 'Me'
  // Strip trailing commas/spaces so we always render exactly one comma.
  const hello = escapeHtml((greeting?.trim() || DEFAULT_GREETING).replace(/[,\s]+$/, ''))
  const rows = items
    .map(
      (item, index) => `
        <tr>
          ...
          <td style="border:1px solid #d9e2ec;padding:8px;">${escapeHtml(fmtDate(item.targetDate))}</td>
...
export function buildOutlookText(items, summary, name, greeting) {
  const signature = name?.trim() || 'Me'
  const hello = (greeting?.trim() || DEFAULT_GREETING).replace(/[,\s]+$/, '')
  const rows = items
    .map(
      (item, index) =>
        `${index + 1}. ${getStatusIcon(item.status)} ${item.title} | Category: ${item.category || '-'} | Priority: ${item.priority} | Status: ${item.status} | Target: ${fmtDate(item.targetDate)} | Notes: ${item.notes || '-'}`,
    )
    .join('\n')
